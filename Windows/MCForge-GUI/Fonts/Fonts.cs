...
namespace MCForge.World.Drawing
{
    public class Fonts
    {
        ...
        unsafe static Fonts()
        {
            ...
            fixed (byte* data = Resource.Ubuntu)
                fonts.AddMemoryFont((IntPtr)data, Resource.Ubuntu.Length);

            fixed (byte* data = Resource.minecraft)
                fonts.AddMemoryFont((IntPtr)data, Resource.minecraft.Length);
